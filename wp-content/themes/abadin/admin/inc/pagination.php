...
function wpthemess_paginate($args = null) {
    ...
    if ($pages > 1) {
        ...
        if ($page > 1 && !empty($previouspage)) {
            $output .= "<a href='" . get_pagenum_link($page - 1) . "' class='emm-prev tooltip' title='previous page'>$previouspage</a>";
        }
        ...
        if ($page < $pages && !empty($nextpage)) {
            $output .= "<a href='" . get_pagenum_link($page + 1) . "' class='emm-next tooltip' title='next page'>$nextpage</a>";
        }
...
function wpthemess_paginate_loop($start, $max, $page = 0) {
    ...
    for ($i = $start; $i <= $max; $i++) {
        $output .= ($page === intval($i))
            ? "<span class='emm-current tooltip' title='page $i'>$i</span>"
            : "<a href='" . get_pagenum_link($i) . "' class='emm-page tooltip' title='page $i'>$i</a>";
    }
    return $output;
}
?>